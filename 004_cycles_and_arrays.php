...
for ($i=1 ; $i < 10 ; ++$i){
    $boringToys += ['name' => 'Toy ' . $i , 'price' => random_int(100 , 1000)];
    print_r($boringToys);
}
...
$colors = ['white' , 'black'  , 'grey' , 'brown' , 'purple' , 'orange' , 'yellow' , 'green' , 'blue'];
$price = random_int(0 , 100);


foreach($cars as $key => $car){
    $cars[0]['colors'] = array_rand(array_flip($colors) , 3 );
    $cars[1]['colors'] = array_rand(array_flip($colors) , 3 );
    $cars[2]['colors'] = array_rand(array_flip($colors) , 3 );
   /*   if ($cars[0]['colors'] = ) {
        $cars[0]['price'] += random_int(0 ,100);
    } */
} 
print_r($cars);
...
foreach($cars as $key => $car){
    ...
    var_dump(' купите авто ' . $car['name'] . ' цвета ' . $color ); 
}
